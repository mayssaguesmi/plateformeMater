        ...
        <div class="bg-white rounded p-4 shadow-sm">
          ...
          <form id="form-profile" method="POST" enctype="multipart/form-data">
            <div class="col-md-12 text-center mb-3">
              ...
              <img  id="avatar-preview" src="<?= esc_url($avatar_url) ?>" class="rounded-circle mb-2" width="90" height="90" alt="Avatar">
...
document.getElementById('form-profile').addEventListener('submit', function (e) {
  ...
  fetch('/wp-json/plateforme-master/v1/update-profile', {
    method: 'POST',
  ...
  .then(data => {
    if (data.success) {
      messageBox.innerHTML = '<div class="alert alert-success">✅ Profil mis à jour.</div>';
      if (data.avatar_url) {
        document.getElementById('avatar-preview').src = data.avatar_url;
      }
    } else {
      messageBox.innerHTML = '<div class="alert alert-danger">❌ Erreur de mise à jour.</div>';
    }
  })
  .catch(() => {
    messageBox.innerHTML = '<div class="alert alert-danger">Erreur réseau.</div>';
  });
});
</script>
