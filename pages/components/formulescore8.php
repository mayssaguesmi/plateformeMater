...
document.querySelector('.utm-save-btn').addEventListener('click', () => {
  ...
  function extractConfig(nom_template, bloc) {
    switch (nom_template) {
      ...
      case 'note_pfe':
        return {
          condition: bloc.querySelector('#pfe-condition')?.value,
          valeur: bloc.querySelector('#pfe-valeur')?.value
        };
      default: {
        const type = bloc.querySelector('input[value="critere_condition"]')?.checked ? 'critere_condition' : 'critere';
        if (type === 'critere_condition') {
          return Array.from(bloc.querySelectorAll('.utm-custom-condition .utm-bonus-row')).map(row => ({
            condition: row.querySelector('input[name*="condition"]')?.value,
            valeur: parseFloat(row.querySelector('input[name*="valeur"]')?.value)
          }));
        }
        return null;
      }
