...
color: #2A2916; color:#2A2916}
          ...
          .msg-subject{font:700 20px/20px Roboto; color:#2A2916}
          ...
          .composer-row{display:flex; align-items:center; gap:14px; padding:0 7px}
    ...
    font-weight: 500;
        ...
        <!-- ===== /NOUVEAU MESSAGE ===== -->
              ...
              <!--  <section class="thread">
                ...
                <!-- ===== NOUVEAU MESSAGE (visible par défaut) ===== -->
<section class="thread" id="nt-section" style="margin-bottom:12px;">
  <div class="inner">
       <div class="msg-header">
        <div class="msg-subject">Nouveau message</div>
      </div>
    ...
    <div class="msg-card" id="nt-card">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:8px; margin-bottom:8px;">
          <div>
            ...
            <select id="nt-role" class="composer-input" style="height:44px;">
              ...
              <option value="um_directeur_laboratoire">Directeur de laboratoire</option>
            </select>
          </div>
          <div>
            <label for="nt-users" style="display:block; margin-bottom:4px;">Utilisateurs (du rôle)</label>
            <select id="nt-users" multiple size="6" class="composer-input" style="height:auto; padding:6px 12px;"></select>
          </div>
        </div>
      ...
      <div class="composer" style="padding-top:0;    text-align: right;">
        <div class="composer-row">
                    <small id="nt-hint" style="opacity:.8;"></small>
...
<!-- ===== DÉTAILS DU FIL (caché par défaut) ===== -->
<section class="thread hidden" id="thread-section">
  <div class="inner">
    <div class="teacher" style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
      <span id="mt-head">—</span>
      <!-- Bouton retour vers “Nouveau message” -->
      <button id="btn-new" class="icon-btn" title="Nouveau message" aria-label="Nouveau message">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
  ...
  <div class="thread-body">
    <div class="inner">
      ...
      <div class="msg-card" id="mt-card">
        <div class="msg-header">
          <div id="mt-subject" class="msg-subject">—</div>
          <div id="mt-date" class="msg-date">—</div>
        </div>
        <div id="mt-content" class="msg-content"></div>
      </div>
...
(function(){
  window.toast = function(msg, isError){
    const hint   = document.getElementById('nt-hint');
    const status = document.getElementById('nt-status');
    if (hint) {
      hint.textContent = String(msg ?? '');
      hint.style.color = isError ? '#b60303' : '';
    } else {
      console.warn('[toast]', msg);
    }
    if (status) {
      try { status.textContent = new Date().toLocaleString('fr-FR'); } catch {}
    }
  };
})();
</script>
...
/*
  const toast = (m, err=false) => {
    ...
    if (hint) {
      hint.textContent = m;
      hint.style.color = err ? '#b60303' : '';
    } else {
      console.warn('[toast]', m); // fallback console si l’élément n’est pas là
    }
    if (status) {
      try { status.textContent = new Date().toLocaleString('fr-FR'); } catch {}
    }
  ...
  function renderThreads() {
    ...
    threads.forEach(t => {
      ...
      <div class="item-top">
        ...
        <div class="date">${esc(t.last_message_at_display || '')}</div>
      </div>
  ...
  async function refreshThreads(){
    try{
      const data = await apiFetch('/messages/threads', {
        method:'GET',
        query: { only_unread: (currentFilter==='unread')?1:0, query: currentSearch, limit: 50, offset: 0 }
      });
 ...
 async function openThread(threadId){
  showThreadDetails(); // <-- affiche la section détails
  currentThreadId = threadId;
  window.currentThreadId = threadId;   // <-- AJOUT : visible depuis sendReply
  ...
  // sujet en haut
  const th = threads.find(t => t.id === currentThreadId);
  subjEl.textContent = th?.subject || '—';

  // dernier message = pour la date
  const last = messages.length ? messages[messages.length - 1] : null;
  if (last) {
    dateEl.textContent = new Date(last.created_at)
      .toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' });
  } else {
    dateEl.textContent = '—';
  }

  // ⚠️ on n'affiche rien dans l'entête (pas de contenu du message ici)
  contEl.innerHTML = '';

  // nettoyer l'historique dynamique
  const anchor = $('#mt-card', threadRoot).parentElement;
  anchor.parentElement.querySelectorAll('.msg-card.__dyn').forEach(n => n.remove());

  // rendre la liste SANS le dernier (déjà “utilisé” pour la date)
  messages.forEach(m => {
    if (last && m.id === last.id) return; // ← évite le doublon
    ...
    wrap.innerHTML = `
      <div class="msg-header">
        <div class="msg-subject">
          <span class="role">${esc(m.sender_role_label || '')}</span>
          &nbsp;<strong>${esc(m.sender_name || '')}</strong>
        </div>
        <div class="msg-date">${esc(m.created_at_display || '')}</div>
      </div>
      <div class="msg-content">${m.body_html ? m.body_html : esc(m.body_text || '')}</div>
      ${files}
    `;
  ...
  async function postMessage(){
    ...
    try{
      ...
      refreshThreads();
    }catch(e){ window.toast(e.message || 'Échec envoi', true); }
  }
...
(function(){
  ...
  async function sendNewThread(){
    ...
    if (!subject) return window.toast('Sujet requis', true);
    if (!body)    return window.toast('Message requis','error');
    if (!sel.length) return window.toast('Sélectionnez au moins un utilisateur','error');

    try{
      sendEl.disabled = true; window.toast('Envoi en cours…');
      const payload = {
        subject,
        participant_ids: sel,
      ...
      window.toast('Fil créé avec succès');
      ...
      window.toast(e.message || 'Échec de création', true);
    }finally{
...
const newSection    = document.getElementById('nt-section');
...
function showNewThread(){
  ...
  // reset éventuels du champ d'envoi détail
...
btnNew?.addEventListener('click', showNewThread);

// Au chargement, on force l'affichage du formulaire
document.addEventListener('DOMContentLoaded', showNewThread);
...
<script>
(function(){
  // ---- Config légère (si tu n'as pas déjà une fonction api() globale) ----
  const API_BASE = (window.PMSettings?.restUrl || '/wp-json').replace(/\/$/,'');
  const NS_MSG   = '/plateforme-messagerie/v1';
  const NONCE    = window.PMSettings?.nonce || (window.wpApiSettings?.nonce || '');

  // API helper (utilise window.apiFetch si dispo)
  const api = (typeof window.apiFetch === 'function')
    ? window.apiFetch
    : async function(path, {method='GET', data=null, query=null, headers={}}={}) {
        const url = new URL(path.startsWith('http')? path : (API_BASE + path), location.href);
  ...
  // ---- Toast robuste (ne plante jamais si #nt-hint ou #nt-status manquent) ----
  ...
  // ---- Charge les utilisateurs d'un rôle ----
  async function loadUsersByRole(role){
    const usersEl = document.getElementById('nt-users');   // récupère à chaque appel
    if (!usersEl) {
      console.warn('[loadUsersByRole] #nt-users introuvable');
      toast('Zone utilisateurs introuvable', true);
      return;
    }

    usersEl.innerHTML = '';
    if (!role) {
      usersEl.innerHTML = '<option value="">(sélectionnez un rôle)</option>';
      return;
    }
    ...
    try{
      ...
      if (!users.length) {
        ...
        toast('Aucun utilisateur pour ce rôle');
        return;
      }
      ...
      console.error('[users-by-role]', e);
      usersEl.innerHTML = `<option value="">Chargement impossible (${e.message || 'erreur'})</option>`;
      toast(e.message || 'Erreur chargement', true);
    }
  }

  // ---- Initialisation DOM (un seul endroit) ----
  document.addEventListener('DOMContentLoaded', ()=>{
    const roleEl = document.getElementById('nt-role');

    if (!roleEl) {
      console.warn('[init] #nt-role introuvable');
      return;
    }
    ...
    // Précharger si une valeur est déjà sélectionnée
    if (roleEl.value) loadUsersByRole(roleEl.value);
  });
...
(function () {
  ...
  const inputEl   = threadRoot.querySelector('#mt-input');
  const sendBtn   = threadRoot.querySelector('#mt-send');
  const attachBtn = threadRoot.querySelector('#mt-attach');
  const card      = threadRoot.querySelector('#mt-card');
  const anchor    = card ? card.parentElement : null;
  if (!inputEl || !sendBtn || !anchor) return;

  // API helper
  const REST  = (window.PMSettings?.restUrl || '/wp-json').replace(/\/$/,'') + '/plateforme-messagerie/v1';
  const NONCE = window.PMSettings?.nonce || (window.wpApiSettings?.nonce || '');
  const api   = (typeof window.apiFetch === 'function')
    ? window.apiFetch
    : async function(path, {method='GET', data=null, query=null, headers={}}={}) {
        const url = new URL(path.startsWith('http')? path : (REST + path), location.href);
  ...
  function appendMine(htmlBody, files=[]) {
    const wrap = document.createElement('div');
    wrap.className = 'msg-card __dyn';
    wrap.style.marginTop = '10px';
    const filesHtml = files.length
      ? `<div style="margin-top:6px">${files.map(a=>`<span>${esc(a.name||a.file_name||'pièce jointe')}</span>`).join(' ')}</div>`
      : '';
    wrap.innerHTML = `
      <div class="msg-header">
        <div class="msg-subject">Vous</div>
      ...
      <div class="msg-content">${htmlBody}</div>
      ${filesHtml}
    `;
    anchor.appendChild(wrap);
    const body = threadRoot.querySelector('.thread-body');
    if (body) body.scrollTop = body.scrollHeight;
  }

  function resolveThreadId(){
    if (window.currentThreadId) return window.currentThreadId;  // posé dans openThread
    const active = document.querySelector('#ml-list .item.active');
    if (active?.dataset?.id) return Number(active.dataset.id);
    return null;
  }

  async function sendReply() {

    console.log('test ilen');
    const threadId = resolveThreadId();
    if (!threadId) { window.toast?.('Sélectionnez un fil', true); return; }

    const raw = inputEl.value || '';
    const trimmed = raw.trim();
    if (!trimmed && (!window.pendingAttachments || !window.pendingAttachments.length)) return;
    ...
    try {
      await api(`/messages/threads/${threadId}/messages`, {
        method: 'POST',
        data: { body: bodyHtml, attachments: (window.pendingAttachments || []) }
      });

      appendMine(bodyHtml, window.pendingAttachments || []);
      inputEl.value = '';
      ...
      window.toast?.(e.message || 'Échec envoi', true);
  ...
  attachBtn?.addEventListener('click', ()=>{
    const fi = document.createElement('input');
      ...
      for (const f of files) {
        ...
        (window.pendingAttachments ||= []).push({ name:f.name, mime:f.type||'application/octet-stream', size:f.size, content:b64 });
      }
      window.toast?.(`${window.pendingAttachments.length} pièce(s) jointe(s) prête(s)`);
    };
    fi.click();
  });
