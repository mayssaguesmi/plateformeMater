...
<head>
    ...
    <title>Suivi des réponses - Menu déroulant corrigé</title>
...
<body>
    <section class="card" id="demandes-service">
        <div class="section-title">
            <span class="title-icon"><i class="fas fa-desktop"></i></span>
            Suivi des réponses
        </div>
        ...
        <div class="table-container">
            <!-- The entire table structure is now self-contained for DataTables -->
            <table class="tbl" id="reclamationsTable">
                <thead>
                    <tr>
        ...
        <!-- The pagination footer will be moved here by the script -->
    ...
    $(document).ready(function() {
        ...
        // This block handles the custom page number display between pagination buttons.

        // Function to update the page number indicator
        function updatePageIndicator() {
            // DataTables redraws pagination on change, removing our custom span.
            // So, we must remove the old one (if any) and re-add it on every draw.
            $('.paginate_page_indicator').remove();

            const pageInfo = table.page.info();
            const pageIndicatorHTML = `<span class="paginate_page_indicator">${pageInfo.page + 1}</span>`;

            // Insert the indicator after the 'previous' button.
            $('.dataTables_paginate .paginate_button.previous').after(pageIndicatorHTML);
        }

        // Update the page indicator on each table draw (e.g., page change).
        ...
        /* =================================================================
            ORIGINAL: Menus ⋯ - FIXED Z-INDEX ISSUE
        ================================================================= */
        // Use event delegation for kebab menus for rows that might be redrawn by DataTable
        $('#reclamationsTable').on('click', '.kebab', function(e) {
            e.stopPropagation();
        ...
        // Use event delegation for "Lire la réponse" links
        $('#reclamationsTable').on('click', '.act-read', function(e) {
            e.preventDefault();
            const tr = $(this).closest('tr').closest('tr'); // Get the parent tr
            $('.dropdown').hide(); // Close dropdown menu
            openSBForRow(tr[0]);
        });
    });
    </script>
