        ...
        <tbody>
            <tr>
                ...
                <td><span class="badge badge-success"> <i class="fa-regular fa-circle-check"            
                            style="color: #0E962D; padding-right:5px;"></i>Terminé</span>
                </td>
                <td>
                    <div class="actions">
    ...
    <div class="popup-container" id="popupContainerObjectifs">
        ...
        <form class="popup-form">
            ...
            <div class="form-group">
                ...
                <div class="input-with-icon">
                    <input type="date" id="activiteDate" placeholder="">
                </div>
            </div>

            <div class="form-group">
                <label for="activiteHeure">Heure De Début / Fin</label>
                <div class="input-with-icon">
                    <input type="text" id="activiteHeure" placeholder="ex: 08:30 – 10:00">
                </div>
            </div>
...
<div class="modal-overlay" id="modalModifier" style="display: none;">
    <div class="popup-container" id="popupContainerModifier">
        ...
        <form class="popup-form">
            ...
            <div class="form-group">
                <label for="editActiviteHeure">Heure De Début / Fin</label>
                <div class="input-with-icon">
                    <input type="text" id="editActiviteHeure" placeholder="ex: 08:30 – 10:00">
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $('#btnSaveObjectifs').on('click', function(event) {
        ...
        // Simple validation
        if (!$('#membreConcerne').val() || !$('#typeActivite').val() || !$('#statutActivite').val()) {
        ...
        const newRowData = [
            ...
            $('#typeActivite').val(),
            $('#fileUpload').files[0] ? $('#fileUpload').files[0].name : '_',
            `<span class="badge badge-secondary">${$('#statutActivite').val()}</span>`, // Simplified status badge
            `<div class="actions">
                <button class="action-btn">...</button>
                <div class="dropdown-menu">
                    <a href="#" data-action="Modifier">Modifier</a>
    ...
    function openEditModal(row) {
        ...
        if (modalModifier) {
            ...
            $('#editActiviteHeure').val(rowData[1]);
    ...
    $('#btnSaveModifier').on('click', function(event) {
        ...
        if (editingRow) {
            const updatedData = [
                ...
                $('#editActiviteHeure').val(),
                $('#editMembreConcerne').val(),
                $('#editDescriptionDetaillee').val(),
                $('#editTypeActivite').val(),
                $('#editFileUpload').files[0] ? $('#editFileUpload').files[0].name : $(
                    '#editFileText').val(),
                `<span class="badge badge-secondary">${$('#editStatutActivite').val()}</span>`, // Simplified status badge
