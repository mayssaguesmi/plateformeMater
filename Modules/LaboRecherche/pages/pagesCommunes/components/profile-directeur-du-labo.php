...
<head>
    ...
    * {
    ...
    .field.select-like .chev {
        position: absolute;
        right: 12px;
        pointer-events: none
    }

    .field.select-like .split {
        position: absolute;
        right: 36px;
        top: 50%;
    ...
    .field.is-locked select:disabled {
        cursor: not-allowed;
        color: #555
    }

    .field.is-locked .chev {
        opacity: .4
    }
...
<body>
    ...
    <div class="wrapper">
        <div class="card">
            <div class="card-header">
                ...
                <button id="btnOpenPwd" class="btn-primary" type="button"><i class="fa-solid fa-key"></i> Modifier votre
                    mot de passe</button>
            </div>
            ...
            <form class="card-body" method="post" action="save_profile.php" enctype="multipart/form-data">
                <div class="form">
                    ...
                    <div class="grid">
                        ...
                        <div class="field is-locked" title="Champ non modifiable">
                            <input type="text" name="prenom" value="<?= htmlspecialchars($profil['prenom']) ?>"
                                placeholder="Prénom" readonly aria-readonly="true">
                        </div>

                        <!-- Nom (verrouillé) -->
                        <div class="field is-locked" title="Champ non modifiable">
                            <input type="text" name="nom" value="<?= htmlspecialchars($profil['nom']) ?>"
                                placeholder="Nom" readonly aria-readonly="true">
                        </div>
                        ...
                        <div class="field select-like is-locked" title="Champ non modifiable">
                            <select name="nationalite_display" disabled aria-disabled="true">
                                <option <?= $profil['nationalite'] === 'Tunisienne' ? 'selected' : ''; ?>>Tunisienne
                                </option>
                                <option <?= $profil['nationalite'] === 'Française' ? 'selected' : ''; ?>>Française
                                </option>
                                <option <?= $profil['nationalite'] === 'Marocaine' ? 'selected' : ''; ?>>Marocaine
                                </option>
                                <option <?= $profil['nationalite'] === 'Algérienne' ? 'selected' : ''; ?>>Algérienne
                                </option>
                            </select>
                            <!-- On envoie quand même la valeur au serveur -->
                            <input type="hidden" name="nationalite"
                                value="<?= htmlspecialchars($profil['nationalite']) ?>">
                        ...
                        <!-- CIN (verrouillé) -->
                        <div class="field is-locked" title="Champ non modifiable">
                            <input type="text" name="cin" value="<?= htmlspecialchars($profil['cin']) ?>"
                                placeholder="CIN / Identifiant" readonly aria-readonly="true">
                        </div>

                        <!-- Emails (modifiables) -->
                        <div class="field"><input type="email" name="email1"
                                value="<?= htmlspecialchars($profil['email1']) ?>" placeholder="Email 1"></div>
                        <div class="field"><input type="email" name="email2"
                                value="<?= htmlspecialchars($profil['email2']) ?>" placeholder="Email 2"></div>
                    </div>
                            ...
                            <div class="phone">
                                <div class="field" id="countryBox" style="position:relative;">
                                    ...
                                    <div class="country-menu" id="countryMenu">
                                        <button type="button" data-iso="tn"><img class="flag"
                                                src="https://flagcdn.com/w20/tn.png" alt=""> Tunisie</button>
                                        <button type="button" data-iso="fr"><img class="flag"
                                                src="https://flagcdn.com/w20/fr.png" alt=""> France</button>
                                        <button type="button" data-iso="ma"><img class="flag"
                                                src="https://flagcdn.com/w20/ma.png" alt=""> Maroc</button>
                                        <button type="button" data-iso="dz"><img class="flag"
                                                src="https://flagcdn.com/w20/dz.png" alt=""> Algérie</button>
                                    </div>
                                </div>
                                <div class="field"><input type="text" name="tel"
                                        value="<?= htmlspecialchars($profil['tel']) ?>" placeholder="+216 ..."></div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="full-row-inner">
                            <div class="file-row">
                                <div class="field" id="cvBox">
                                    <input type="text" id="cvName" value="<?= htmlspecialchars($profil['cv']) ?>"
                    ...
                    <div class="address-block">
                        <div class="field">
                            <input type="text" name="adr_etud" value="<?= htmlspecialchars($profil['adr_etud']) ?>"
                                placeholder="Adresse étudiant">
                        </div>
                        <div class="address-row">
                            <div class="field select-like">
                                <select name="gov_etud" id="gov_etud">
                                    <option value="" <?= $profil['gov_etud'] === '' ? 'selected' : ''; ?>>Gouvernorat
                                    </option>
                                    <option <?= $profil['gov_etud'] === 'Ariana' ? 'selected' : ''; ?>>Ariana</option>
                                    <option <?= $profil['gov_etud'] === 'Ben Arous' ? 'selected' : ''; ?>>Ben Arous
                                    </option>
                            ...
                            <div class="field">
                                <input type="text" name="cp_etud" value="<?= htmlspecialchars($profil['cp_etud']) ?>"
                    ...
                    <div class="address-block">
                        <div class="field">
                            <input type="text" name="adr_parents"
                                value="<?= htmlspecialchars($profil['adr_parents']) ?>" placeholder="Adresse parents">
                        </div>
                        <div class="address-row">
                            <div class="field select-like">
                                <select name="gov_parents" id="gov_parents">
                                    <option value="" <?= ($profil['gov_parents'] ?? '') === '' ? 'selected' : ''; ?>>
                                        Gouvernorat</option>
                                    <option <?= ($profil['gov_parents'] ?? '') === 'Ariana' ? 'selected' : ''; ?>>Ariana
                                    </option>
                                    <option <?= ($profil['gov_parents'] ?? '') === 'Ben Arous' ? 'selected' : ''; ?>>Ben
                                        Arous
                                    </option>
                                    <option <?= ($profil['gov_parents'] ?? '') === 'Manouba' ? 'selected' : ''; ?>>
                                        Manouba
                                    </option>
                                    <option <?= ($profil['gov_parents'] ?? '') === 'Tunis' ? 'selected' : ''; ?>>Tunis
                                    </option>
                                </select>
                                <span class="split" aria-hidden="true"></span>
                                <i class="fa-solid fa-chevron-down chev" aria-hidden="true"></i>
                            ...
                            <div class="field">
                                <input type="text" name="cp_parents"
                                    value="<?= htmlspecialchars($profil['cp_parents']) ?>" placeholder="Code postal">
                            </div>
                        </div>
                        ...
                        <div class="phone" style="margin-top:4px;">
                            ...
                            <div class="field">
                                <input type="text" name="tel_parents"
                                    value="<?= htmlspecialchars($profil['tel_parents']) ?>" placeholder="+216 ...">
                            </div>
                        </div>
                    </div>
    ...
    countryBox.addEventListener('click', () => countryMenu.style.display = (countryMenu.style.display === 'block' ?
        'none' : 'block'));
    ...
    document.addEventListener('click', e => {
        if (!countryBox.contains(e.target)) countryMenu.style.display = 'none';
    });
    ...
    countryBoxParents.addEventListener('click', () => countryMenuParents.style.display = (countryMenuParents.style
    ...
    document.addEventListener('click', e => {
        if (!countryBoxParents.contains(e.target)) countryMenuParents.style.display = 'none';
    });
