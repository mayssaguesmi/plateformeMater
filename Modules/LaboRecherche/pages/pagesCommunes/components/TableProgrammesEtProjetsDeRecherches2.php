...
.popup-form .form-group .input-with-icon {
    position: relative;
}

.popup-form .form-group .input-with-icon img {
    position: absolute;
    right: 12px;
    top: 50%;
    transform: translateY(-50%);
...
(() => {
    ...
    const openModal = (edit = false, project = null) => {
        ...
        if (project) {
            ...
            // Dates
            if (project.date_debut) inputs.date_debut.value = project.date_debut.split('T')[0];
            if (project.date_fin) inputs.date_fin.value = project.date_fin.split('T')[0];
    ...
    const applyFilters = () => {
        ...
        filtered = projects.filter(p => {
            ...
            // Use the same logic for status as in the display to handle default cases correctly.
            const effectiveStatus = (p.statut || r.statut || 'En cours').toLowerCase();
        ...
        if (a) {
            ...
            if (a.classList.contains('btn-modifier')) {
                ...
                wpFetch(`/projet/${id}`)
                    ...
                    .catch(err => {
                        notify('Erreur chargement projet : ' + err.message, 'error');
                    });
                return;
            }
