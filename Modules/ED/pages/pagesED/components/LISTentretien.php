...
<div class="content-block">
  ...
  <div class="header-bar">
      <h4 style="margin: 0; display: flex; align-items: center;">
        ...
        Liste de candidatures validées
      </h4>
  </div>
...
<script>
document.addEventListener('DOMContentLoaded', () => {
  fetch('/wp-json/plateforme-master/v1/entretiens-valides')
    .then(res => res.json())
    .then(data => {
      ...
      data.forEach(row => {
        ...
        const etatEntretien = row.etat_entretien === 'Déjà envoyé' ? 'Déjà envoyé' : '–';

        const tdActions = `
          <td>
            <div class="actions">
              <button class="action-btn" data-candidature="${row.candidature_id}">...</button>
              <div class="dropdown-menu" style="display: none;">
                <a href="#" class="fixer-entretien" data-candidature="${row.candidature_id}">
                  <i class="fa fa-calendar"></i> Fixer un entretien
                </a>
                 <a href="#" class="evaluer-entretien" data-candidature="${row.candidature_id}">
                  <i class="fa fa-edit"></i> Évaluation du candidat
                </a>
                <a href="/fiche-candidature?id=${row.candidature_id}">
                  <i class="fa fa-eye"></i> Consulter dossier
                </a>
              </div>
            </div>
          </td>
        ...
        tr.innerHTML = `
          <td><input type="checkbox" data-id="${row.candidature_id}"></td>
          <td>${row.etudiant}</td>
          <td><span class="badge badge-success"><i class="fa fa-check-circle"></i> Validé</span></td>
      ...
      // 🔹 Activer le menu déroulant (bouton ...)
      ...
      // 🔹 Ouvrir le modal entretien
      document.querySelectorAll('.fixer-entretien').forEach(btn => {
        btn.addEventListener('click', function (e) {
          e.preventDefault();
          const id = this.getAttribute('data-candidature');
          openEntretienModal(id);
        });
      });
      // 🔹 Ouvrir le modal d’évaluation
      document.querySelectorAll('.evaluer-entretien').forEach(btn => {
        btn.addEventListener('click', function (e) {
          e.preventDefault();
          const id = this.getAttribute('data-candidature');
          openEvaluationModal(id); // fonction déjà définie dans le script précédent
        });
      });

    })
    .catch(err => console.error("Erreur chargement entretiens :", err));
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Ferme le modal quand on clique à l’extérieur du panneau
