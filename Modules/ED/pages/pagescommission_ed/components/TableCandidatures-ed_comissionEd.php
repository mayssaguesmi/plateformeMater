<div class="content-block">
  <div class="header-bar">
    <h2>
      <img src="/wp-content/plugins/plateforme-master/images/ed/3655586.png" alt="Icon" style="width: 28px; margin-right: 8px; vertical-align: middle;">
      Tableau de gestion des candidatures
    </h2>
  </div>
  ...
  <div class="filter-bar">
    <div class="filter-selectgb">
      ...
      <div class="filters-row">
        <div class="control control-select">
          <select class="filter-select">
            <option>Ecole Doctorale</option>
            <option>ED Sciences</option>
            <option>ED Médecine</option>
          </select>
          <i class="fa fa-chevron-down icon-right"></i>
        </div>
    ...
    <div class="filter-actions">
      <!-- Statut dropdown (rouge) -->
      <div class="statut-dropdown-wrapper">
        <button type="button" class="statut-dropdown-btn">
          Statut <i class="fa fa-chevron-down" style="margin-left:8px;"></i>
        </button>
        <div class="statut-dropdown-menu">
          <div class="statut-dropdown-option">Valider</div>
          <div class="statut-dropdown-option">Dossier Incomplet</div>
          <div class="statut-dropdown-option">Refusé</div>
        </div>
      </div>

      <!-- Icônes (tri / filtres) -->
      <button class="icon-btn" title="Trier"><i class="fa fa-sort"></i></button>
      <button class="icon-btn" title="Filtres"><i class="fa fa-filter"></i></button>
    </div>
  </div>
  ...
  <table id="candidaturesTable" class="styled-table display" style="width:100%;">
    ...
    <tbody>
      <tr>
        ...
        <td>Yassine Messaoudi</td>
        <td>Physique</td>
        <td>03/06/2025</td>
        <td>Pr. Hatem Sassi</td>
        ...
        <td>
          <div class="actions">
            ...
            <div class="dropdown-menu">
              <a href="/fiche-candidatures-ed_comissionEd/"><i class="fa fa-eye"></i> Voir dossier</a>
              <a href="#"><i class="fa fa-bell"></i> Relancer</a>
      ...
      <tr>
        ...
        <td>Biologie</td>
        <td>03/06/2025</td>
        <td>Dr. Salma Jouini</td>
        <td><span class="badge badge-warning">Incomplet</span></td>
        <td>Accepté</td>
        <td><i class="fa fa-paperclip"></i> <span class="doc-count">6</span></td>
        <td>
          <div class="actions">
            <button class="action-btn">⋮</button>
            <div class="dropdown-menu">
      ...
      <tr>
        ...
        <td>Rejeté</td>
        <td><i class="fa fa-paperclip"></i> <span class="doc-count">2</span></td>
        <td>
          <div class="actions">
            <button class="action-btn">⋮</button>
            <div class="dropdown-menu">
  ...
  <!-- Pagination style maquette (DataTables la remplira) -->
</div>
...
/* Conteneur global des filtres (2 lignes) */
.filter-selectgb{
  display:flex;
...
/* Une ligne = groupe horizontal de contrôles */
.filters-row{
  display:flex;
  align-items:center;
  gap:10px;
...
/* Contrôle générique (pills) */
.control{
  position:relative;
  display:flex;
  align-items:center;
  background:#fff;
  border:2px solid #dcdac2;         /* beige doux */
  border-radius:14px;                /* arrondi pill */
  height:42px;
  padding:0 14px;
  box-shadow:0 0 6px rgba(0,0,0,0.03);
}

/* Recherche : largeur plus grande */
.control-search{
     min-width: 240px;
    max-width: 240px;
  flex:0 1 240px;
...
/* Selects : largeur fixe (comme la maquette) */
.control-select{
  min-width:240px;
  flex:0 0 240px;
}
...
.filter-select{
  ...
  outline:none;
  background:transparent;
  padding-right:22px;                /* place pour chevron */
  font-size:14px;
  color:#888;
  width:100%;
  cursor:pointer;
...
/* Focus states (subtile rouge UTM) */
.control:has(.search-input:focus),
.control:has(.filter-select:focus){
  border-color:#d71920;
  box-shadow:0 0 0 3px rgba(215,25,32,0.08);
}

/* Responsive : passe tout sur une seule colonne si étroit */
@media (max-width: 980px){
  .filters-row{ flex-direction:column; align-items:stretch; }
  .control-search, .control-select{ flex:1 1 auto; min-width:100%; }
}
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const closeAll = () => {
      ...
      document.querySelectorAll('.statut-dropdown-menu').forEach(m => m.style.display = 'none');
    };

    // Gestion unifiée des menus: bouton "⋮" et bouton "Statut"
    document.addEventListener('click', function (e) {
      const actionBtn = e.target.closest('.action-btn');
      const statutBtn = e.target.closest('.statut-dropdown-btn');
      const insideMenu = e.target.closest('.dropdown-menu, .statut-dropdown-menu');

      if (actionBtn) {
        e.preventDefault();
        const menu = actionBtn.parentElement.querySelector('.dropdown-menu');
      ...
      if (statutBtn) {
        e.preventDefault();
        const wrapper = statutBtn.closest('.statut-dropdown-wrapper');
        const menu = wrapper ? wrapper.querySelector('.statut-dropdown-menu') : null;
        const visible = menu && menu.style.display === 'block';
        closeAll();
        if (menu) menu.style.display = visible ? 'none' : 'block';
        return;
